...
def calculate_difference(actual_df, calculated_df):
    ...
    st.write(difference)
    st.write("Total Difference:", indian_accounting_format(difference_sum))
...
def display_interest_and_principle_emi(actual_df):
    ...
    st.write("Monthly Interest Rate:", get_percentage_format(monthly_IR))
    st.write("Annual Interest Rate:", get_percentage_format(annual_IR))
    st.write("Principle:", indian_accounting_format(principle))
    st.write("EMI:", indian_accounting_format(emi))
